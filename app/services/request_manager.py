...
from app.constants.webdriver import (
    ...
    MESSAGE,
    MESSAGE_DETAIL,
    PINTEREST_CLIENT_CONTEXT,
    PINTEREST_RESOURCE_RESPONSE,
    STATUS,
    SUCCESS,
)
...
class RequestManager:
    ...
    def __init__(
        self, csrf_token: str, cookies: dict[str, str], extension_domain: str
    ) -> None:
        ...
        self.__cookies: dict[str, Any] = (
            cookies  # self.__convert_cookies(cookies)
        )
    ...
    def get_request_body(
        self, source_url: str, data: dict[str, Any]
    ) -> dict[str, str]:
        """Generate the body for the query in dictionary format.
    ...
    def format_response(
        self, response: Response, resource_response: bool
    ) -> dict[str, Any]:
    ...
    def __format_error(self, error: dict[str, Any]) -> dict[str, Any]:
        ...
        """
        __error_detail: str = (
            (": " + error[MESSAGE_DETAIL]) if MESSAGE_DETAIL in error else ""
        )
        return {ERROR: error[MESSAGE] + __error_detail, CODE: error[CODE]}

    def request_error(
        self, response: dict[str, Any], error_message: str
    ) -> None:
        """Handle errors in upload requests by checking the response status.
