...
from app.constants.request_body import (
    ...
    PAGES,
    PAID_PIN_URL,
    PIN_CONTENT_ORGANIC_BODY,
    PIN_CONTENT_PAID_BODY,
    PIN_IMAGE_SIGNATURE,
    PIN_TITLE,
    PINBOARD_ID_BODY,
    SCHEDULED_TIMESTAMP,
...
class UploadBody:
    ...
    def __amazon_credentials_organic(
        ...
        amazon_credentials[DATA][OPTIONS][DATA][MEDIA_INFO_LIST] = dumps(
            media_info
        )  # Add the string format of the media info list.
    ...
    def __amazon_credentials_paid(self, file_type: str) -> dict[str, Any]:
        ...
        amazon_credentials[DATA][OPTIONS][TYPE] = (
            IMAGE_PAID_PIN if IMAGE_PIN_TYPE == file_type else VIDEO_PAID_PIN
        )
        return amazon_credentials

    def get_amazon_credentials_body(
        self, uuids: tuple[str, str], file_type: str, paid_pin: bool
    ) -> dict[str, Any]:
        """Generate a JSON request body for obtaining Amazon S3 credentials.
    ...
    def __pin_content_organic(
        self, content: dict[str, Any], asset_etag: str, preview_etag: str
    ) -> dict[str, Any]:
        """Generate a JSON request body for uploading organic Pin content.
        ...
        signature = (
            IMAGE_SIGNATURE if asset_etag == preview_etag else VIDEO_SIGNATURE
        )  # Asset key signature (image or video).
