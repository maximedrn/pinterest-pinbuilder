...
class CreateManager(FileWriter):
    ...
    def get_assets_preview_binary(self) -> list[str]:
        ...
        __max_index: int = min(
            __min_index + self.__chunk_size, len(self.__pins_data)
        )  # Prevent IndexError.
        return [
            AssetsManager.get_asset_binary(self.__pins_data[index][FILE_PATH])
            for index in range(__min_index, __max_index)
        ]
    ...
    def apply_data_by_index(
        self, index: int, data_from_frontend: dict[str, Any]
    ) -> None:
        """Apply data from the frontend to a specific pin.
