...
class BaseManager:
    ...
    def is_process_running(self, process_name: str | None = None) -> bool:
        ...
        """
        return (
            any(self[process] for process in PROCESSES)
            if not process_name
...
class Interface(BaseManager):
    ...
    def get_assets_preview_binary(self) -> list[str] | list:
        ...
        """
        return (
            self.__create.get_assets_preview_binary()
    ...
    def load_previous_chunk(self) -> bool:
        ...
        """
        return (
            self.__create.load_previous_chunk()
            if self.__is_creation_started()
            else False
        )
    ...
    def __stop_process(
        self, process_name: str, process_manager: ProcessManager | None
    ) -> None:
    ...
    def start_upload_process(
        ...
        return self.__start_process(  # Send the function to call and the
            kwargs,
            UPLOAD_PROCESS,  # different arguments to start the
            self.__upload_manager.start_upload_process,
        )  # upload.
