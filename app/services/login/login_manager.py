# -*- coding: utf-8 -*-
# app/services/login/login_manager.py
...
class LoginManager(WebdriverManager):
    ...
    def __is_user_logged(self) -> None:
        ...
        """
        WebDriverWait(
            driver=self.driver, timeout=self.__logging_timeout,
    ...
    def __call__(self, manager: DictProxy[Any, Any]) -> None:
        ...
        try:
            ...
            while self.__attempts < self.__attempts_limit and not \
                    CookieManager.verify_cookies(self.__session_cookies):
            ...
            user_data: UserManager.UserData = UserManager()\
                .retrieve_user_data(self.__session_cookies)
