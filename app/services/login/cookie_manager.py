...
class CookieManager:
    ...
    def verify_cookies(cookies: dict[str, Any]) -> bool:
        ...
        """
        return (
            PINTEREST_SESSION_COOKIE in cookies
            and PINTEREST_CSRF_COOKIE in cookies
        )

    @staticmethod
    def same_uuid(
        cookies: dict[str, Any], cookies_: dict[str, Any]
    ) -> bool | None:
        """Check if two sets of cookies have the same UUID or session cookie.
    ...
    def format_cookies(
        ...
        __csrf_cookie: str = (
            __cookies[PINTEREST_CSRF_COOKIE]
            if PINTEREST_CSRF_COOKIE in cookies
            else ""
        )
