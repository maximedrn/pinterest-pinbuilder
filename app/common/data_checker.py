...
from app.constants.messages import (
    ...
    DATETIME_PAST_ERROR,
    DATETIME_SCHEDULE_ERROR,
    EXTERNAL_LINK_ERROR,
    FILE_PATH_ERROR,
    FILE_PATH_VALUE_ERROR,
    PIN,
    PINBOARD_VALUE_ERROR,
    STRING_LENGTH_ERROR,
    TOPIC_TAGS_ERROR,
)
...
class DataChecker:
    ...
    def __check_string_length(self) -> None:
        ...
        for key_name, max_length in DATA_LENGTH.items():
            value: str = (
                self.__current_data[key_name]
                if not self.__missing(key_name)
                else ""
            )
            if value and len(value) > max_length:
                raise IncorrectValueError(
                    STRING_LENGTH_ERROR.format(
    ...
    def __check_pinboard(self) -> None:
        ...
        """
        __is_paid: bool = (
            bool(self.__current_data[PAID_PIN])
            if not self.__missing(PAID_PIN)
            else False
        )
    ...
    def __check_external_link(self) -> None:
        ...
        """
        external_link: str = (
            self.__current_data[LINK] if not self.__missing(LINK) else ""
        )
    ...
    def __check_topic_tags(self) -> None:
        ...
        topic_tags: list[dict[str, str]] = (
            self.__current_data[TOPIC_TAGS]
            if not self.__missing(TOPIC_TAGS)
            else []
        )
    ...
    def __check_datetime(self) -> None:
        ...
        """
        __datetime_string: str = (
            self.__current_data[DATETIME]
            if not self.__missing(DATETIME)
            else ""
        )
